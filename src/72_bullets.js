/* ---------- what the horn fires ----------------------------------
   Kinds: 0 plain, 1 pierces everything, 2 goo (leaves a puddle when it
   lands), 3 frost (slows what it hits). b: bounces left, p: things it may
   still pass through. Every dying shot paints the meadow a little.    */
function shot(x, y, an, sp, d, hue, l, k) {
  bu.push({ x, y, vx: cos(an) * sp, vy: sin(an) * sp, d: d * P.dmg, c: hue, l: l * P.life, k, b: P.bounce ? 1 : 0, p: k == 1 ? 99 : P.pierce + (mech ? 3 : 0) });
}
function volley() {
