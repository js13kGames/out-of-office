...
function title() {
  ...
  txt(touch ? 'LEFT: MOVE   RIGHT: AIM AND FIRE   TAP LEFT: BLINK' : 'WASD MOVE   MOUSE AIM   HOLD TO FIRE   SHIFT: BLINK', W / 2, 142, C(0, 70, 0), 1, 1);
...
function perkScreen() {
  ...
  opts.forEach((o, i) => {
    const x = cardX(i), on = i == sel, r = o[2], rc = RC[r], leg = r == 4;
  ...
  txt(touch ? 'TAP ONE' : 'ARROWS AND ENTER, OR 1 2 3', W / 2, y + 100, C(0, 60, 0), 1, 1);
}
function overScreen() {
